...
void rotate_point(Point * p, Point * origin) {
...
void rotate_polygon(Polygon * p) {
    ...
    for (int i = 0; i < p->vertices->num_objects; i++) {
        centroid.x += ((Point*)all_points[i]->object)->x;
        centroid.y += ((Point*)all_points[i]->object)->y;
    }
    ...
    for (int i = 0; i < p->vertices->num_objects; i++) {
        rotate_point((Point*)all_points[i]->object, &centroid);
    }
}
